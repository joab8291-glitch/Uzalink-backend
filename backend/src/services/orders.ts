...
export async function createOrder(
  ...
  if (product.inventory !== null && product.inventory < 1) {
    throw new Error("Product is out of stock");
  }

  const commission = Math.round(
    (product.priceCents * COMMISSION) / 100
  );
  ...
  return prisma.order.create({
    data: {
      ...
      metadata: buyer.address
        ? { address: buyer.address }
        : undefined,

      amountCents: product.priceCents,
      commissionCents: commission,
      sellerNetCents: product.priceCents - commission,
...
export async function fulfillPaidOrder(
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    await tx.order.update({
      ...
      data: {
        status:
          item.product.kind === "PHYSICAL" ||
          item.product.kind === "SERVICE" ||
          item.product.kind === "BOOKING"
            ? "PAID"
            : "PAID",
    ...
    if (
      item.product.instant &&
      item.product.privateFileKey
    ) {
      ...
      await tx.downloadGrant.create({
        data: {
          ...
          expiresAt: new Date(
            Date.now() +
              item.product.expiresHours * 3600000
          ),
  ...
  const download = result.downloadToken
    ? `\nDownload securely: ${
      ...
      }/api/orders/download/${result.downloadToken}`
    : "";
...
export async function issueDownload(
  ...
  if (!grant) {
    throw new Error("Download link is invalid");
  }

  if (
    grant.order.status !== "PAID" &&
    grant.order.status !== "FULFILLED"
  ) {
    throw new Error("Payment not confirmed");
  }
  ...
  if (!grant.product.privateFileKey) {
    throw new Error(
      "No private file is attached to this product"
    );
  }
